...
class ldapModel extends model
{
    ...
    public function sync2db($config)
    {
        ...
        $user = new stdclass();
        // Save the default permission group information synchronized from LDAP settings
        $group = new stdClass(); 
        $account = '';
        $i=0;
        for (; $i < $ldapUsers['count']; $i++) {        
            ...
            // The default permission group identifier is not stored in LDAP, so it is directly fetched from the config.
            // The ternary operator is used for compatibility with the built-in timing task of Zentao.
